...
namespace PerfTesting.Strings
{
    public class RegexTests
    {
        ...
        [Benchmark]
        public Regex GetCompiled() => new Regex(_needle, RegexOptions.Compiled);

        [Benchmark]
        public Regex GetUncompiled() => new Regex(_needle);

        [Benchmark]
        public void StaticRegexIsMatch()
        {
            var bigMatch = Regex.IsMatch(_ch1TheHobbit, _needle);
            var smallMatch = Regex.IsMatch(_smallHaystack, _needle);
        }

        private static readonly Regex _precompiled = new Regex(_needle, RegexOptions.Compiled);

        [Benchmark]
        public void CachedPrecompiledRegexIsMatch()
        {
            var bigMatch = _precompiled.IsMatch(_ch1TheHobbit);
            var smallMatch = _precompiled.IsMatch(_needle);
        }

        private static readonly Regex _uncompiled = new Regex(_needle);

        [Benchmark]
        public void CachedUncompiledRegexIsMatch()
        {
            var bigMatch = _uncompiled.IsMatch(_ch1TheHobbit);
            var smallMatch = _uncompiled.IsMatch(_needle);
        }

        [Benchmark]
        public void InlineInstantiation()
        {
            var bigMatch = new Regex(_needle).IsMatch(_ch1TheHobbit);
            var smallMatch = new Regex(_needle).IsMatch(_smallHaystack);
        }
...
All the same, I should like it all plain and clear, said he obstinately, putting on his business manner (usually reserved for people who tried to borrow money off him), and doing his best to appear wise and prudent and professional and live up to Gandalf's recommendation. Also I should like to know about risks, out-of-pocket expenses, time required and remuneration, and so forthby which he meant: What am I going to get out of it? and am I going to come back alive?
